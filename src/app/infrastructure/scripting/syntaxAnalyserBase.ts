...
export abstract class SyntaxAnalyserBase {
  ...
  protected get currentToken(): Tokens {
    return this._symbol.token;
  }

  protected isToken(token: Tokens): boolean {
    return this._symbol.token === token;
  }

  protected isNotToken(token: Tokens): boolean {
    return this._symbol.token !== token;
  }
