...
export class BaseGridRenderService {
  ...
  public renderHeader() {
    ...
    const alignment = this.coord.headerAlignment();
  ...
  public drawGridHeader(columns: number): void {
    ...
    for (let col = 0; col < columns; col++) {
      ...
      if (headerCell) {
        const x = this.coord.headerCellX(col);
        headerCtx.drawImage(
          headerCell,
          x,
          0,
  ...
  public drawGridSelectedCell(
    ...
    const col: number = this.coord.cellX(position.column - firstVisibleCol);
    const row: number =
  ...
  private drawFillHandle(
    ...
    const handleX = this.coord.fillHandleX(cellX);
  ...
  private drawSelectedCellBackground(
    ...
    const column: number = this.coord.cellX(col - firstVisibleCol);
    const rowSet: number =
