  ...
  createContextMenu(context: ContextMenuContext): Menu {
    ...
    if (isCellFilled) {
      ...
      const isLocked = entry
        ? (entry.lockLevel > 0 || entry.isGroupRestricted || context.dataService.isCellOutsideMembershipPeriod(context.row, context.column) || context.dataService.isCellOutsideGroupPeriod(context.row, context.column))
        : context.dataService.isCellOutsideMembershipPeriod(context.row, context.column) || context.dataService.isCellOutsideGroupPeriod(context.row, context.column);
    ...
    } else if (context.dataService.isColumnSealed(context.column) || context.dataService.isCellOutsideMembershipPeriod(context.row, context.column) || context.dataService.isCellOutsideGroupPeriod(context.row, context.column)) {
      return menuData;
    } else {
