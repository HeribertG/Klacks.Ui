...
describe('GanttPdfDrawingService', () => {
    ...
    beforeEach(() => {
        ...
        mockHolidaysHelper = {
            getTotalDaysInCurrentYear: vi.fn(),
            getDaysInMonth: vi.fn()
        };
        mockHolidaysHelper.getTotalDaysInCurrentYear.mockReturnValue(365);
        mockHolidaysHelper.getDaysInMonth.mockReturnValue(31); // Default to 31 days
        ...
        mockPdf.getTextWidth.mockReturnValue(50);
    });
    ...
    describe('getDaysInYear', () => {
        ...
        it('should return 366 for leap year', () => {
            mockHolidaysHelper.getTotalDaysInCurrentYear.mockReturnValue(366);
    ...
    describe('drawRowHeader', () => {
        it('should draw row header with correct styling', () => {
            ...
            expect(mockPdf.rect).toHaveBeenCalledWith(10, 20, expect.any(Number), 30, 'F');
            ...
            expect(mockPdf.text).toHaveBeenCalledWith('Test Client', 15, expect.any(Number));
        });
        ...
        it('should draw top border when requested', () => {
            ...
            const lineCalls = vi.mocked(mockPdf.line).mock.calls;
    ...
    describe('drawMonthHeaders', () => {
        it('should draw month headers with translated names', () => {
            ...
            const rectCalls = vi.mocked(mockPdf.rect).mock.calls;
            expect(rectCalls.length).toBeGreaterThan(0);

            // Should draw month names
            const textCalls = vi.mocked(mockPdf.text).mock.calls;
    ...
    describe('drawRowSeparatorLine', () => {
        it('should draw horizontal line across full width', () => {
            ...
            expect(mockPdf.setDrawColor).toHaveBeenCalled();
            expect(mockPdf.setLineWidth).toHaveBeenCalledWith(expect.any(Number));
    ...
    describe('drawLegend', () => {
        it('should draw legend items with colors and names', () => {
            ...
            expect(mockPdf.setFillColor).toHaveBeenCalled();
            expect(vi.mocked(mockPdf.setFillColor).mock.calls.length).toBeGreaterThanOrEqual(2);

            // Should draw text labels
            expect(mockPdf.text).toHaveBeenCalledWith('Vacation', expect.any(Number), expect.any(Number));
            expect(mockPdf.text).toHaveBeenCalledWith('Sick Leave', expect.any(Number), expect.any(Number));
        });
    ...
    describe('drawCompleteRow', () => {
        it('should draw all row components', () => {
            ...
            vi.spyOn(service, 'drawRowHeader');
            vi.spyOn(service, 'drawRowBackground');
            vi.spyOn(service, 'drawRowBreaks');
            vi.spyOn(service, 'drawRowSeparatorLine');
        ...
        it('should include month headers when requested', () => {
            ...
            vi.spyOn(service, 'drawMonthHeaders');
