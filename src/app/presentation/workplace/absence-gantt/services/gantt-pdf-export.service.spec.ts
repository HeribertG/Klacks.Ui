...
describe('GanttPdfExportService', () => {
    ...
    let mockGanttPdfDrawingService: any;
    let mockPdf: any;
    ...
    beforeEach(() => {
        ...
        const dataAbsenceSpy = {
            absenceList: vi.fn()
        };
        const drawingSpy = {
            createDefaultConfig: vi.fn(),
            drawLegend: vi.fn(),
            drawMonthHeaders: vi.fn(),
            drawRowSeparatorLine: vi.fn(),
            drawCompleteRow: vi.fn(),
            getDaysInYear: vi.fn()
        };

        // Set up mock returns
        translateSpy.instant.mockReturnValue('Translated');
        dataBreakSpy.readClientName.mockReturnValue('Test Client');
        dataBreakSpy.readData.mockReturnValue([]);
        dataAbsenceSpy.absenceList.mockReturnValue([]);
        drawingSpy.createDefaultConfig.mockReturnValue({
            pageWidth: 1190.55,
            pageHeight: 841.89,
        ...
        drawingSpy.getDaysInYear.mockReturnValue(365);

        TestBed.configureTestingModule({
            providers: [
                GanttPdfExportService,
                { provide: TranslateService, useValue: translateSpy },
                { provide: DataManagementBreakPlaceholderService, useValue: dataBreakSpy },
                {
        ...
        mockDataManagementBreak = TestBed.inject(DataManagementBreakPlaceholderService) as any;
        mockDataManagementAbsence = TestBed.inject(DataManagementAbsenceGanttService) as any;
        mockGanttPdfDrawingService = TestBed.inject(GanttPdfDrawingService) as any;
    ...
    describe('getSelectedAbsenceTypes', () => {
        it('should return selected absence types with localized names', () => {
            ...
            mockDataManagementAbsence.absenceList.mockReturnValue([
                {
                    id: '1',
    ...
    describe('addPageHeader', () => {
        it('should add title and page information', () => {
            ...
            expect(mockTranslateService.instant).toHaveBeenCalledWith('pdf.generated');
    ...
    describe('exportTest2DDrawing', () => {
        ...
        it('should use current year from break filter in title', async () => {
            ...
            expect(mockPdf.text).toHaveBeenCalledWith(expect.stringContaining('Gantt Chart 2024'), expect.any(Number), expect.any(Number));
        });
        ...
        it('should use custom title if provided', async () => {
            ...
            expect(mockPdf.text).toHaveBeenCalledWith(expect.stringContaining('Custom Export Title'), expect.any(Number), expect.any(Number));
        });
        ...
        it('should draw rows for each client', async () => {
            ...
            mockDataManagementBreak.readClientName.mockReturnValueOnce('Client 1').mockReturnValueOnce('Client 2').mockReturnValueOnce('Client 3');
            mockDataManagementBreak.readData.mockReturnValue([]);

            await service.exportTest2DDrawing();

            expect(mockGanttPdfDrawingService.drawCompleteRow).toHaveBeenCalledTimes(3);
        });
        ...
        it('should handle pagination for many clients', async () => {
            ...
            mockDataManagementBreak.readClientName.mockReturnValue('Client');
            mockDataManagementBreak.readData.mockReturnValue([]);
        ...
        it('should pass break data to drawCompleteRow', async () => {
            ...
            mockDataManagementBreak.readData.mockReturnValue(mockBreaks);

            await service.exportTest2DDrawing();

            expect(mockGanttPdfDrawingService.drawCompleteRow).toHaveBeenCalledWith(mockPdf, expect.any(Number), expect.any(Number), expect.any(Object), expect.any(String), false, 20, expect.any(Boolean), expect.any(Boolean), mockBreaks);
        });
        ...
        it('should display message when no client data available', async () => {
            ...
            expect(mockPdf.text).toHaveBeenCalledWith('No client data available', expect.any(Number), expect.any(Number));
            expect(mockPdf.text).toHaveBeenCalledWith('Please ensure that data has been loaded', expect.any(Number), expect.any(Number));
        });
        ...
        it('should display summary when clients are exported', async () => {
            ...
            expect(mockPdf.text).toHaveBeenCalledWith(expect.stringContaining('Total 5 clients exported'), expect.any(Number), expect.any(Number));
        ...
        it('should use year from break filter for config', async () => {
            ...
            expect(mockPdf.text).toHaveBeenCalledWith(expect.stringContaining('Year: 2025'), expect.any(Number), expect.any(Number));
        });
    });
