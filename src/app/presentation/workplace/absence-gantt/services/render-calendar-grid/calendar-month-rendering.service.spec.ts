...
describe('CalendarMonthRenderingService', () => {
  ...
  let mockHolidayCollection: jasmine.SpyObj<HolidayCollectionService>;
  let mockCalendarSetting: jasmine.SpyObj<CalendarSettingService>;
  let mockGridSetting: jasmine.SpyObj<GridSettingsService>;
  let mockGridFonts: jasmine.SpyObj<GridFontsService>;
  let mockTranslateService: jasmine.SpyObj<TranslateService>;
  let mockBackgroundRowCtx: jasmine.SpyObj<CanvasRenderingContext2D>;

  let mockCurrentYear: number;

  beforeEach(() => {
    mockBackgroundRowCtx = jasmine.createSpyObj('CanvasRenderingContext2D', [
      'save',
      'restore',
      'fillRect',
      'clearRect',
    ...
    mockHolidayCollection = jasmine.createSpyObj('HolidayCollectionService', ['dummy']);
    ...
    mockGridSetting = jasmine.createSpyObj('GridSettingsService', [], {
      showGridLines: true,
    });
  ...
  describe('isCanvasAvailable', () => {
    ...
    it('should return false when canvas is not available', () => {
      mockGanttCanvasManager.isCanvasAvailable.and.returnValue(false);
  ...
  describe('drawMonthBackgrounds', () => {
    ...
    it('should handle leap year correctly', () => {
      ...
      const expectedMarchLeft =
        (januaryDays + februaryDays) * mockCalendarSetting.cellWidth;
    ...
    it('should add extra cellWidth to month width', () => {
      ...
      const januaryDays = 31;
      const expectedWidth =
        januaryDays * mockCalendarSetting.cellWidth +
  ...
  describe('Month color alternation', () => {
    it('should use even color for January (month 0)', () => {
      ...
      const firstFillCall = mockBackgroundRowCtx.fillRect.calls.first();
    ...
    it('should use odd color for February (month 1)', () => {
      ...
      expect(mockBackgroundRowCtx.fillRect.calls.count()).toBe(12);
    });
  });
  ...
  describe('Month positioning', () => {
    ...
    it('should span full year width', () => {
      ...
      const totalDays = 366;
      const expectedMaxRight =
        totalDays * mockCalendarSetting.cellWidth +
        mockCalendarSetting.cellWidth;
