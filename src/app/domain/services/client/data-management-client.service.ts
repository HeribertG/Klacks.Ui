...
import { Filter, IClient, IClientAttribute } from 'src/app/domain/models/client-class';
...
export class DataManagementClientService implements ISaveable, IResettable, ILoadable {
  ...
  public readonly paginationDataService = this.clientListService.paginationDataService;
  public readonly headerCheckBoxValue = this.clientListService.headerCheckBoxValue;
  public readonly editClient = this.clientEditService.editClient;
  public readonly currentAddressIndex = this.clientEditService.currentAddressIndex;
  public readonly currentAnnotationIndex = this.clientEditService.currentAnnotationIndex;
  ...
  public readonly findClientMaxPages = this.clientSearchService.findClientMaxPages;
  public readonly findClientMaxVisiblePage = this.clientSearchService.findClientMaxVisiblePage;
  public readonly communicationPhoneList = computed(() => this.editClient()?.communications.filter(c => c.isPhone) || []);
  public readonly communicationEmailList = computed(() => this.editClient()?.communications.filter(c => c.isEmail) || []);
  public readonly stateList = this.clientConfigService.stateList;
  public readonly communicationPrefixList = this.clientConfigService.communicationPrefixList;
  public readonly communicationTypePhoneList = this.clientConfigService.communicationTypePhoneList;
  public readonly communicationTypeEmailList = this.clientConfigService.communicationTypeEmailList;
  ...
  public showProgressSpinner = computed(() => 
    this.clientListService.showProgressSpinner() || this.clientEditService.showProgressSpinner()
  );
  ...
  constructor() {
    ...
    effect(() => {
      if (this.clientConfigService.isInit()) {
        ...
        this.currentFilter.filteredStateToken = this.clientConfigService.stateList();
  ...
  public deleteClient = (key: string) => this.clientListService.deleteClient(key);
  public exportExcel = (type = 0) => this.clientListService.exportExcel(this.currentFilter);
  public clearCheckedArray = () => this.clientListService.clearCheckedArray();
  public addCheckBoxValueToArray = (value: any) => this.clientListService.addCheckBoxValueToArray(value);
  public findCheckBoxValue = (key: string) => this.clientListService.findCheckBoxValue(key);
  public removeCheckBoxValueToArray = (key: string) => this.clientListService.removeCheckBoxValueToArray(key);
  public checkBoxIndeterminate = () => this.clientListService.checkBoxIndeterminate();
  
  public createClient = () => this.clientEditService.createClient();
  public readClient = (id: string) => this.clientEditService.readClient(id);
  public prepareClient = (value: IClient, withoutUpdateDummy = false) => this.clientEditService.prepareClient(value, withoutUpdateDummy);
  public addAnnotation = () => this.clientEditService.addAnnotation();
  public removeCurrentAnnotation = () => this.clientEditService.removeCurrentAnnotation();
  public readClientAddressListWithoutQueryFilter = () => this.clientEditService.readClientAddressListWithoutQueryFilter();
  ...
  public readActualSortedClientPage = () => this.clientSearchService.readActualSortedClientPage();
  ...
  public addPhone = () => {
    if (this.editClient()) {
      this.clientEditService.editClient.set(this.communicationService.addPhone(this.editClient()!));
    }
  }
  public delPhone = (index: number) => {
    if (this.editClient()) {
      this.clientEditService.editClient.set(this.communicationService.delPhone(this.editClient()!, index));
    }
  }
  public addEmail = () => {
    if (this.editClient()) {
      this.clientEditService.editClient.set(this.communicationService.addEmail(this.editClient()!));
    }
  }
  public delEmail = (index: number) => {
    if (this.editClient()) {
      this.clientEditService.editClient.set(this.communicationService.delEmail(this.editClient()!, index));
    }
  }

  public removeCurrentAddress = () => {
    if (this.editClient()) {
      const updatedClient = this.addressService.removeCurrentAddress(this.editClient()!, this.currentAddressIndex());
  ...
  public writeCity = async () => {
    if (this.editClient()) {
      ...
      const { address: newAddress, lastCountries, stateList } = await this.addressService.writeCity(address, this.stateList());
  ...
  public filterState = () => {
    if (this.editClient()) {
      const address = this.editClient()!.addresses[this.currentAddressIndex()];
      const filteredStates = this.addressService.filterState(address, this.clientConfigService.stateList());
  ...
  public isCurrentAddressMainAddress = () => {
    if (this.editClient()) {
      return this.addressService.isCurrentAddressMainAddress(this.editClient()!, this.currentAddressIndex());
    }
    return false;
  }
  ...
  public reReadAddress = () => {
    if (this.editClient()) {
      const { editClient, currentAddressIndex } = this.addressService.setAddress(this.editClient()!, this.currentAddressIndex());
  ...
  readChangeList(locale: string = MessageLibrary.DEFAULT_LANG) {
    this.dataClientService
      ...
      .subscribe((x) => {
        ...
        this.subTitleLastChanges.set(`${DateToString(
          x.lastChange,
          locale
