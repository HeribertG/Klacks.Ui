...
@Injectable()
export class AssistantExecutionBranchService {

  executeCreateBranch(call: IAssistantFunctionCall): Observable<IAssistantFunctionResult> {
    return from(this.doCreateBranch(call));
  }

  private async doCreateBranch(call: IAssistantFunctionCall): Promise<IAssistantFunctionResult> {
  ...
  executeDeleteBranch(call: IAssistantFunctionCall): Observable<IAssistantFunctionResult> {
    return from(this.doDeleteBranch(call));
  }

  private async doDeleteBranch(call: IAssistantFunctionCall): Promise<IAssistantFunctionResult> {
  ...
  executeListBranches(call: IAssistantFunctionCall): Observable<IAssistantFunctionResult> {
    return from(this.doListBranches(call));
  }

  private async doListBranches(call: IAssistantFunctionCall): Promise<IAssistantFunctionResult> {
    let branchHeader = document.getElementById('branches-table-header');
    if (!branchHeader) {
      document.getElementById('open-settings')?.click();
      branchHeader = await waitForElement('branches-table-header', 5000);
