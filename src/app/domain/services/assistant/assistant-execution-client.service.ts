...
@Injectable()
export class AssistantExecutionClientService {
  ...
  executeCreateClient(call: IAssistantFunctionCall): Observable<IAssistantFunctionResult> {
    const {
      firstName,
      lastName,
      gender,
    ...
    return from(this.ensureServicesInitialized()).pipe(
      ...
      switchMap(() => {
        return new Observable<IAssistantFunctionResult>((observer) => {
