...
export class LLMFunctionExecutionService {
  ...
  executeFunction(
    ...
    switch (functionCall.name) {
      ...
      // UI-Aktionen: Macros
      case 'create_macro':
        return this.executeCreateMacro(functionCall);
      case 'delete_macro':
        return this.executeDeleteMacro(functionCall);
      case 'list_macros':
        return this.executeListMacros(functionCall);
  ...
  private executeCreateMacro(
    call: ILLMFunctionCall
  ): Observable<ILLMFunctionResult> {
    return from(this.doCreateMacro(call));
  }

  private async doCreateMacro(call: ILLMFunctionCall): Promise<ILLMFunctionResult> {
    const { name, type, content } = call.arguments;

    let macrosSection = document.getElementById('settings-macros');
    if (!macrosSection) {
      document.getElementById('open-settings')?.click();
      macrosSection = await this.waitForElement('settings-macros', 5000);
      if (!macrosSection) {
  ...
  private executeListMacros(
    call: ILLMFunctionCall
  ): Observable<ILLMFunctionResult> {
    return from(this.doListMacros(call));
  }

  private async doListMacros(call: ILLMFunctionCall): Promise<ILLMFunctionResult> {
    let macrosSection = document.getElementById('settings-macros');
    if (!macrosSection) {
      document.getElementById('open-settings')?.click();
      macrosSection = await this.waitForElement('settings-macros', 5000);
